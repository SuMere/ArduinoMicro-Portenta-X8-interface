...
namespace SudoMaker::chAT {
	...
	public:
		...
		void write_raw(const data_holder& d) {
		...
		void write_data(const void *buf, size_t len) {
			write_raw(data_holder{(uint8_t *) buf, len, 0});
		}
		...
		void write_str(std::string str) {
			write_raw(data_holder{nullptr, 0, 0, std::move(str)});
		}
		void write_vec8(std::vector<uint8_t> vec8) {
			write_raw(data_holder{nullptr, 0, 0, std::move(vec8)});
		}
